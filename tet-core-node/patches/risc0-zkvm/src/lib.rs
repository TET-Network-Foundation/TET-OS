...
mod claim;
pub mod guest;
// Host pulls RV emulator (`rrs-lib`) + `std` stacks unsuitable for Substrate `wasm32-unknown-unknown`.
#[cfg(all(not(target_os = "zkvm"), not(target_arch = "wasm32")))]
mod host;
mod mmr;
mod receipt;
pub mod serde;
...
#[cfg(not(target_os = "zkvm"))]
pub use risc0_binfmt::compute_image_id;

#[cfg(not(target_os = "zkvm"))]
pub use risc0_groth16::Seal as Groth16Seal;
