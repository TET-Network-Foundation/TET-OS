...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	...
	// Bump when runtime semantics change so apps refresh metadata / types.
	spec_version: 108,
	impl_version: 1,
...
pub const MILLI_UNIT: Balance = 100_000;
pub const MICRO_UNIT: Balance = 100;

/// Existential deposit.
// Keep low for UX in early phases (prevents accidental reaping of small accounts).
...
/// No `ChargeTransactionPayment`; no `CheckMetadataHash` / `WeightReclaim` (Polkadot.js SCALE alignment).
...
where
	RuntimeCall: From<LocalCall> + Encode,
	LocalCall: Encode + Member,
{
	...
	LocalCall: Encode + Member,
{
	fn create_signed_transaction<C: frame_system::offchain::AppCrypto<Self::Public, Self::Signature>>(
		...
		let signed_payload = SignedPayload::new(call, tx_ext).ok()?;
		let signature = signed_payload.using_encoded(|payload| C::sign(payload, public))?;
